                            ...
                            tcp_helper_resource_t* trs)
{
  ...
  if( NI_OPTS(ni).prealloc_packets && trs->thc_efct_alloc ) {
    ...
    if( n_rxqs ) {
      ...
      rc = hugetlb_pages_prealloc(trs->thc_efct_alloc, nr_pages);
      if( rc < 0 ) {
        if( rc == -ENOSPC )
                        ...
                        tcp_helper_resource_t** rs_out)
{
  ...
  rs->thc_efct_alloc = hugetlb_allocator_create(alloc->in_efct_memfd);
  ...
  if( thc && thc->thc_pktbuf_alloc ) {
    rs->thc_pktbuf_alloc = hugetlb_allocator_get(thc->thc_pktbuf_alloc);
  } else {
    rs->thc_pktbuf_alloc = hugetlb_allocator_create(alloc->in_pktbuf_memfd);
    ...
    if( thc && rs->thc_pktbuf_alloc )
      thc->thc_pktbuf_alloc = hugetlb_allocator_get(rs->thc_pktbuf_alloc);
  }
 ...
 fail6:
  ...
  if( rs->thc_pktbuf_alloc )
    hugetlb_allocator_put(rs->thc_pktbuf_alloc);

 fail_pktbuf_memfd:
  if( rs->thc_efct_alloc )
    hugetlb_allocator_put(rs->thc_efct_alloc);
 fail_efct_memfd:
#if CI_CFG_NIC_RESET_SUPPORT
...
void tcp_helper_dtor(tcp_helper_resource_t* trs)
{
  ...
  if( trs->thc_pktbuf_alloc )
    hugetlb_allocator_put(trs->thc_pktbuf_alloc);
  if( trs->thc_efct_alloc )
    hugetlb_allocator_put(trs->thc_efct_alloc);
...
efab_tcp_helper_more_bufs(tcp_helper_resource_t* trs)
{
  ...
  struct oo_buffer_pages* pages;
#ifdef OO_DO_HUGE_PAGES
  struct hugetlb_page *hugetlb_page;
#endif
  uint64_t *hw_addrs;
  ci_irqlock_state_t lock_flags;
  ci_netif* ni = &trs->netif;
