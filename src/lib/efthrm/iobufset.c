...
static void oo_iobufset_free_pages(struct oo_buffer_pages *pages)
{
  ...
  if( hugetlb_page_valid(&pages->hugetlb_page) ) {
    hugetlb_page_free(&pages->hugetlb_page);
  } else
#endif
  {
    int i;
                           ...
                           int n_bufs, int gfp_flag)
{
  ...
  pages->n_bufs = n_bufs;
#ifdef OO_DO_HUGE_PAGES
  hugetlb_page_reset(&pages->hugetlb_page);
#endif
                            ...
                            struct hugetlb_allocator *hugetlb_alloc)
{
  ...
  if( (*flags & (OO_IOBUFSET_FLAG_HUGE_PAGE_TRY |
    ...
    if( hugetlb_alloc ) {
      rc = hugetlb_page_alloc(hugetlb_alloc, &pages->hugetlb_page);
      if( ! rc ) {
